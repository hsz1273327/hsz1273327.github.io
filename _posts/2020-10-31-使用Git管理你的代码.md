...
主干分支策略的特点是主干上,分支用于发布.所有人在主干上开发意味着每次提交代码都需要做代码合并操作,小步快跑可以避免长时间隔离开发后合并代码没有头绪的问题(但是开发人员会很厌烦频繁合并代码).通常主干分支上开发新功能是通过代码级别的开关激活的,这样也可以避免不稳定的功能过早引入发布版本造成问题(虽然不优雅).
...
1. 必须要有整套稳定的CI/CD解决方案,这个不是开发团队的问题,但这往往是最打的问题,国内多数企业的对开发的概念还停留在上个世纪,没有意识到这个东西的强悍
...
+ master分支时常保持着软件可以正常稳定运行的状态.由于要维护这一状态,所以不允许开发者直接对master分支的代码进行修改和提交.和Github Flow一样,需要提交`Pull Request`
...
+ 功能有冗余,比如`release`分支完全可以使用tag替代
+ Github Flow中存在的长周期特性的问题一样没有解决.

总结来说:

+ Git Flow比较适合较大的有专门流程管理的团队
