...
而更多的时候我们会组成`组织`以和一些人一起完成一些项目.对于`组织`来说,成员需要被划分为`团队`,`团队`的作用类似linux中的`group`用户组,一般是按功能划分的.同一个`团队`中的成员有相同的基础权限.而添加新的成员也必须从`团队`中尽心添加
...
三种团队的成员中只有所有者或管理员团队的成员才能创建新的团队.一个组织只可以维护自己内部的团队,如果你希望引入组织成员外部的人员来参与个别项目,也可以像在私人仓库中一样通过引入外部的`协作者`来实现.而对于分支的保护,我们一样是使用`分支保护`功能控制,只是其中增加可以用团队来控制的条件.
