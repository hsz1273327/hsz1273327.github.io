...
> 一些可以打开文件夹的应用(比如vscode等),默认我们想在文件管理器中右键打开时需要在`右键菜单->打开->打开方式`中在最下面的其他程序中进行选择,埋的太深了.我们可以将这些应用添加到默认右键菜单中.方法是进入`~/.local/share/applications`目录,找到对应应用的`.desktop`文件,编辑它,在其中添加一行`MimeType=inode/directory;`,保存后重新登录即可生效.
