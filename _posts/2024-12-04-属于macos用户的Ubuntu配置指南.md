...
`MOC (Machine Owner Key)`是一种用于在 Linux 系统上签署内核模块的机制.它允许用户创建和管理自己的密钥对,并使用这些密钥对内核模块进行签名.这样,即使是第三方或自定义的内核模块,也可以被系统信任并加载,而不会被安全机制阻止.一般比较高端的主板BIOS都会支持这个功能,但低端主板可能不支持.

如果是支持`MOC`的主板,无论是amd显卡还是N卡,第一次安装显卡驱动过程中会让你填一个密码,密码要求6~8位.这个密码即`MOK`,它是用来给驱动签名的,如果你不填这个密码,驱动是无法加载的.在设置好密码后,重启时会进入一个蓝色的界面让你选,注意,**默认第一个选项是让你直接进系统,不能选**,选第二个`Enroll MOK`,一路跟着引导最后会让你输入这个密码,输入后驱动就可以正常加载了.
...
这几乎是一切网络操作的基础,ubuntu默认并不会安装curl,因此我们需要先安装它
...
很多必备工具的安装都依赖于`flatpak`,因此我们需要先安装它
...
安装好后市面上大部分的音频视频格式我们就都可以使用了,注意安装过程中会有一个`ttf-mscorefonts-installer`的EULA协议,需要用`Tab`键选中`<OK>`然后回车接受协议.我们也可以通过`sudo dpkg-reconfigure ttf-mscorefonts-installer`来重新配置这个协议.我们需要挂好代理才能顺利安装.
...
> 一些可以打开文件夹的应用(比如vscode等),默认我们想在文件管理器中右键打开时需要在`右键菜单->打开->打开方式`中在最下面的其他程序中进行选择,埋的太深了.我们可以将这些应用添加到默认右键菜单中.方法是进入`~/.local/share/applications`或`/usr/share/applications/`目录(取决于你的安装方式),找到对应应用的`.desktop`文件,编辑它,在其中的`MimeType`中添加`inode/directory;`,保存后重新登录即可生效.
...
macos的桌面使用逻辑总体是基于单一桌面的,而gnome的桌面使用逻辑总体是基于多桌面(workspace)的.这两种逻辑并不冲突,但也并不完全相同.我们完全可以沿用macos的使用习惯,但如果能习惯gnome桌面的使用逻辑,其实是可以大大提高效率的.

下面我会介绍两种使用模式,一种是基于单一桌面的,另一种是基于多桌面的.这两种模式用到的技巧和工具都是通用的,只是基于不同的使用习惯有所侧重而已.

### 基于单一桌面使用习惯的优化

在macos中我们习惯了使用单一桌面,而且喜欢将常用的应用固定在dock上.在ubuntu中我们也可以这样做.但在ubuntu中我们可以做进一步优化,可以做到比在macos中更高效.

> 给dock添加点击功能.

`Dash to Dock`扩展支持自定义图标的点击行为,我们可以精细化设置不同的点击行为,让dock的使用效率更高,从而提高单一桌面的使用效率.进入`dash to dock`的设置界面,进入`行为`标签,这里可以设置键盘激活,点击行为和鼠标滚轮行为;在`点击动作`的下级菜单中可以设置`Shift+点击`,`中键点击`和`Shift+中键点击`的行为.下面我给出我个人的设置
...
基于单一桌面的使用模式下我们一旦开的窗口多了就会很乱,这时候我们就需要一个窗口整理工具.在`ubuntu`中我们可以使用插件`Arrange Windows`,它和其他窗口整理工具最大的不同是它不会主动在创建窗口时就进行整理,而是通过快捷键和顶栏菜单来触发整理行为.这样我们就可以在需要的时候才进行整理,而不是每次创建窗口都被打乱.

需要注意的是`Arrange Windows`支持多显示器,但我并不建议真有多显示器时打开,因为打开后如果显示器大小不一样,整理效果会很差.`Arrange Windows`不打开多显示器支持时,它会将所有显示器的窗口都整理到主显示器上,这样就不会出现显示器大小不一样时整理效果差的问题.

### 基于多桌面使用习惯的优化

在gnome中多桌面是一个重要的使用逻辑,我们可以充分利用多桌面来提高工作效率.合理的设置好每个workspace的使用场景,并且合理的使用快捷键来切换workspace,可以大大提高工作效率.如果配合上多显示器使用,效率会更高. workspace是ubuntu自带的功能,我们只需要合理使用即可.

> 固定数量workspace vs 动态数量workspace

在gnome中workspace有两种模式,一种是固定数量workspace,另一种是动态数量workspace.他们可以在`设置->多任务`中的`工作空间`进行切换.

+ 默认是动态数量workspace,这种模式下我们可以随时创建新的workspace,但缺点是无法预知当前有多少个workspace,因此无法通过快捷键直接切换到某个特定的workspace.在工作流不定的情况下,动态数量workspace更适合我们使用.
+ 而固定数量workspace则可以通过快捷键直接切换到某个特定的workspace,但缺点是无法随时创建新的workspace.在工作流比较固定的情况下,固定数量workspace更适合我们使用.

> 多显示器下的workspace

如果你有多个显示器,那么你可以在`设置->多任务`中的`多显示器`中将进行设置.

+ 默认情况下只有主显示器有workspace,副显示器不会随着workspace切换而切换.
+ 如果你希望所有显示器的workspace都随着主显示器的workspace切换而切换,可以将`工作区在所有显示器上`打开.这样所有显示器的workspace都会随着主显示器的workspace切换而切换.
