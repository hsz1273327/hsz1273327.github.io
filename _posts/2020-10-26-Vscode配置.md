...
这套插件安装好后会在左侧增加一个像电脑屏幕一样的图标用于管理远程连接,同时在左下脚会有一个连接的状态只是图标.我们可以点进远程管理图标中选中`ssh targets`,然后通过读取`$Home/.ssh/config`来获取已经注册的ssh连接.对ssh配置不太了解的同学可以去看[关于ssh的技巧]()这篇文章
