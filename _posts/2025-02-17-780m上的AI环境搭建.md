...
因此我们完全可以在bios中的将`VRAM`设置为`auto`(默认为2g)让核显的`vram`仅用于显示,计算就全靠`GTT`.
...
## rocm的选择[2025-11-12更新]

基座运算库有多重要看看老黄赚多少就知道了.cuda早已占据了最好的生态位,这让amd和英特尔的显卡包括核显都很难受.

回到我们的主题,在ubuntu 24.04环境下的780m上我们能用的基座运算库有如下选择:

+ 官方的[rocm](https://github.com/ROCm/ROCm)
+ 第三方的[lamikr/rocm_sdk_builder](https://github.com/lamikr/rocm_sdk_builder/tree/master)
+ 半官方的[ROCm/TheRock](https://github.com/ROCm/TheRock)

不过如果打算使用除pytorch/huggingface套件和llama.cp/ollama外的的其他工具,我们就不能装官方rocm和相关工具,而是要借助`lamikr/rocm_sdk_builder`或`OCm/TheRock`这样的第三方或半官方项目,这个我们后面再说.

## 官方的rocm[2025-11-12更新]
...
我们的780m核显虽然可以安装rocm但需要有额外设置而且很多周边工具并不支持(比如onnxruntime等),当然正常用是没啥问题的,而且pytorch和llama.cpp(及其生态比如ollama)是可以正常使用的.我们装官方rocm的时候需要关注下pytorch支持到的版本.截致2025年11月12日,pytorch稳定版支持的rocm版本最高为`6.4`,但`7,0`版本据说有非常大的性能优化,我们以旧版本的6.2.4为基础介绍,大家举一反三.
...
使用这个镜像只需要要注意将`/dev/kfd`和`/dev/dri`加入`device`并且让容器和用户有一样的组就行
...
## 半官方的`ROCm/TheRock`[2025-11-12更新]

[ROCm/TheRock](https://github.com/ROCm/TheRock)是amd官方主持但由社区维护的一个rocm的衍生项目,`rocm_sdk_builder`的主要贡献者也参与其中,可以看成是`rocm_sdk_builder`的后继项目.这个项目的主要目的是让更多的amd显卡和核显可以被rocm支持,它本质上是一个rocm相关工具链的编译工具加针对未获得官方支持的显卡的补丁集合.经由该项目编译出来的rocm工具链可以直接对应设备上安装使用.

该项目目前还处于早期阶段,但已经展现出了不错的前景,它基本紧跟官方rocm的版本,目前已经支持到rocm 7.0版本,但很遗憾,我没有编译成功,因此也无法提供使用经验.有兴趣的朋友可以自行尝试.
...
~~#### 解锁全量GTT[2025-1-12更新(现在已经不需要了,因为官方已经解决了这个问题)]~~
...
#### 两种基座下ollama性能对比[2025-11-14更新]

由于rocm_sdk_builder版本已经固定,它的性能基本可以看成是一个稳定的基线.而官方rocm版本由于版本更新较快,因此性能会有波动.

目前我的测试结论是在官方的rocm 7.1版本下,ollama在`prefill`阶段和`decoding`阶段的性能互有增减:

+ 在`prefill`阶段,官方rocm 7.1版本下的ollama性能要优于rocm_sdk_builder版本,大约提升了30%左右
+ 在`decoding`阶段,官方rocm 7.1版本下的ollama性能要略逊于rocm_sdk_builder版本,大约降低了5%左右

因此总体来说,如果你的使用场景需要输入较长的上下文,官方rocm版本下的ollama会更好些;如果你的使用场景是短文本对话,或者使用的是thinking模型,或者是要模型输出较多内容的场景,rocm_sdk_builder版本下的ollama会更好些.

但是注意,对于moe模型,亲测无论哪种基座都不如直接cpu用avx跑,其性能是用igpu的两倍.
...
需要注意这个插件默认使用flash-attention2,但780m并不支持flash-attention,我们可以修改其源码--在插件根目录下的`yue_node.py`中把`attn_implementation="flash_attention_2"`都注释掉即可
