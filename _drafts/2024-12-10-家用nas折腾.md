...
+ Fan Start Temperature Limit: 温度多少度时风扇开始转,建议`40`
+ PWM Slope Setting: 温度每升高一度转速提升多少,pwm是0~255表示最小和最大,建议`5`

`Fan Off Temperature Limit`应该是开始计算pwm值的起始位置,`Fan Start Temperature Limit`是真正开始转的位置,因此如果我们按上面我推荐的设置,则起始转速为`(40-15)*5/255 = 125/255 ~= 50%`到60度过热保护时为`(60-15)*5/255 ~= 88%`
