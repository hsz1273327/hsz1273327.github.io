...
sphinx支持使用jinja2语法做扩展,我们可以在`_template`文件夹下写一个`languages.html`(注意由于css使用的是`sphinx_rtd_theme`提供的.因此这个方法只能在这个主题中使用)
...
这个模板可以看出,我们会用到变量`display_lower_left`,`version`,`current_language`,和`languages`,因此我们需要在`config.py`中配置这三个变量
